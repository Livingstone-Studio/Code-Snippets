...
HRESULT Renderer::InitD3D()
{
	...
	hr = m_device->CreateSamplerState(&samplerDesc, &m_skybox_sampler);
	if (FAILED(hr))
	{
		OutputDebugString(L"Failed to create skybox sampler.");
		return E_FAIL;
	}
	...
	result = CreateDDSTextureFromFile(m_device, m_device_context, L"Assets/skybox01.dds", NULL, &m_skybox_texture);
	if (FAILED(result))
	{
		OutputDebugString(L"Failed to load skybox dds.");
		return E_FAIL;
	}
